...
Є повторюваний `seed:demo` для каталогу з 10 товарів, різними залишками та 8 категоріями. Він створює початкові налаштування, variants, options, facet і колекції, не скидаючи наявні price/stock. Після міграцій і build запусти його з явним дозволом і підтвердженням БД — [інструкція та перевірки](./docs/demo-seed.md). Для застосування правил колекцій потрібен звичайний Worker.
